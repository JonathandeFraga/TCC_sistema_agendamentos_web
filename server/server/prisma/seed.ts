...
function moneyBRLToCent(value: string): number {
    const s = value.replace(/[^\d,.-]/g, '').replace('.', '').replace(',', '.');
    const n = Number(s);
    return Math.round(n * 100);
}

function isBlockedDate(d: DateTime, holidaySet: Set<string>): boolean {
    const iso = d.toISODate()!;
    return d.weekday === 1 || d.weekday === 7 || holidaySet.has(iso);
}

function nextValidDays(from: DateTime, count: number, holidaySet: Set<string>): DateTime[] {
    const out: DateTime[] = [];
...
async function main() {
    // 1. Feriados 2026
    const feriados2026: Array<{ data: string; nome: string }> = [
